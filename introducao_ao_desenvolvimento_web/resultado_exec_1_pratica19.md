...
<body>
    <?php
        ...
        $valorFinalFormatadoEmReal = number_format($valorFinal,2,",",".") . "<br>" ;

        $myfile = fopen("arquivo_contem_valor.txt", "w") or die("Unable to open file!");
        fwrite($myfile, $valorFinalFormatadoEmReal);        
        fclose($myfile);

        $myfile = fopen("arquivo_contem_valor.txt", "r") or die("Unable to open file!");
        $paraLeEmTela =  fread($myfile,filesize("arquivo_contem_valor.txt"));
        fclose($myfile);
    ?>
